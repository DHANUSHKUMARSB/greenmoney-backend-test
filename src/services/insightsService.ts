export const detectSpendingPatterns = (transactions: any[]) => {
...
export const generateSavingsSuggestions = (transactions: any[]) => {
  const suggestions = [];
  ...
  if (sortedCategories.length > 0) {
    ...
    suggestions.push(`Reduce ${highestCat[0]} expenses by 15% to save $${reduceTarget.toFixed(2)}/month.`);
  }
...
export const detectAnomalies = (transactions: any[]) => {
  ...
  transactions.filter(t => t.type === 'expense').forEach(tx => {
    if (tx.amount > mean + 2 * stdDev && tx.amount > 50) { // filter out very small noise anomalies
      anomalies.push(`Anomaly: Unusually high expense of $${tx.amount.toFixed(2)} on ${tx.category_name || 'Unknown'} (${new Date(tx.date).toLocaleDateString()}).`);
    }
  });
...
export const generateAllInsights = (transactions: any[]) => {
  return {
    patterns: detectSpendingPatterns(transactions),
    suggestions: generateSavingsSuggestions(transactions),
