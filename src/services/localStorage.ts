...
export const localStorage = {
  ...
  _update: async <T extends BaseEntity>(key: string, id: string, updates: Partial<T>, isFromSync = false): Promise<void> => {
    const release = await dbLock.acquire();
    try {
      const items = await localStorage._getItems<T>(key);
      ...
      if (!isFromSync) syncEvents.emitMutation();
    } finally { release(); }
  },
  ...
  getTransactionById: async (id: string) => (await localStorage.getTransactions()).find(t => t.id === id),
  ...
  updateBudget: (id: string, updates: any, isFromSync = false) => localStorage._update<Budget>(KEYS.BUDGETS, id, updates, isFromSync),
