  ...
  try {
    ...
    for (const template of activeTemplates) {
      ...
      while (nextDate <= today && template.status === 'active' && iterations < MAX_ITERATIONS) {
        ...
        // Notify user about the automated transaction
        const { scheduleTransactionReminder } = require('./notificationService');
        await scheduleTransactionReminder(
          "🔄 Automated Transaction",
          `Generated ${tx.type}: ${tx.note || 'Recurring'} for ${tx.amount}`,
          new Date()
        );
