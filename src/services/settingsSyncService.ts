...
export const settingsSyncService = {
   ...
   */
  applyCloudSettings(settings: UserSettings) {
    const appStore = useAppStore.getState();
    const themeStore = useThemeStore.getState();
    ...
    if (settings.privacy_settings) appStore.setHideBalance(settings.privacy_settings.hide_balance);
  ...
  async sync(isManual = false) {
    ...
    try {
      ...
      const cloudProfile = await api.syncProfile(user.id, {
