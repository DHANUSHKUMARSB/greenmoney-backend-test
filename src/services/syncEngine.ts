...
export const syncEngine = {
  ...
  async processQueue(isManual = false) {
    ...
    try {
      ...
      const payload: SyncPayload = {
        ...
        ...(useAuthStore.getState().username && { username: useAuthStore.getState().username }),
        ...(useAuthStore.getState().profileImage && { profile_image: useAuthStore.getState().profileImage }),
      };

      // 3. Universal Sync Trip (Atomic Push/Pull)
      const response = await api.universalSync(user.uid, payload);
      ...
      if (updates) {
        ...
        if (updates.username) {
          useAuthStore.getState().setUsername(updates.username);
        }
        if (updates.profile_image) {
          useAuthStore.getState().setProfileImage(updates.profile_image);
        }
      }
      ...
      if (isManual) {
        ...
        toastService.show('Cloud Backup Complete', 'success');
      }

    } catch (error: any) {
      const msg = error.response?.data?.message || error.message;
      logger.error(`[REAL-TIME]: Sync failed: ${msg}`, error.response?.data || error);
  ...
  async updateLocalStatus(collection: string, ids: string[], status: 'synced' | 'failed') {
    const methodMap: any = {
      TRANSACTIONS: { update: localStorage.updateTransaction, key: 'app_transactions_v2', get: localStorage.getTransactions },
      CATEGORIES: { update: localStorage.updateCategory, key: 'app_categories_v2', get: localStorage.getCategories },
      ACCOUNTS: { update: localStorage.updateAccount, key: 'app_accounts_v2', get: localStorage.getAccounts },
      BUDGETS: { update: localStorage.updateBudget, key: 'app_budgets_v2', get: localStorage.getBudgets },
      GOALS: { update: localStorage.updateGoal, key: 'app_goals_v2', get: localStorage.getGoals },
      RECURRING: { update: localStorage.updateRecurring, key: 'app_recurring_v2', get: localStorage.getRecurringTransactions },
    };
    
    const target = methodMap[collection];
    if (!target) return;

    // Get the full list (including soft-deleted) to check for pending deletions
    const allItems = await target.get();
    ...
    for (const id of ids) {
      if (status === 'synced') {
        ...
        if (item?.deleted_at) {
          // Hard delete locally now that it's synced to cloud
          await localStorage._deleteFromStorage(target.key, id);
        } else {
          await target.update(id, { sync_status: 'synced' }, true);
        }
      } else {
        await target.update(id, { sync_status: status }, true);
      }
    }
  },
  ...
  async reconcile(collection: string, cloudItems: BaseEntity[]) {
    const methodMap: any = {
      TRANSACTIONS: { get: (id: string) => localStorage.getTransactionById(id, true), update: localStorage.updateTransaction },
